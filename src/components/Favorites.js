...
export default function Favorites() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      //if favorites contains the id of a product, then add it to the favorites array
      if (cookies.favorites) {
        cookies.favorites.forEach((id) => {
          //find the product with the id in the products array
          const product = res.data.find((product) => product.productId === id);
          //if the product is found, add it to the favorites array
          if (product) {
